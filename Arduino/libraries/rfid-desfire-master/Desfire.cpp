...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_GetApplicationIds(mifare_desfire_tag *tag, mifare_desfire_aid_t *aids, byte *applicationCount)
{
	...
	*applicationCount = aidBufferSize / 3;
	...
	Serial.println("Select aplications");
	Serial.println(aids[0].data[0]);
...
/*
	...
	Funciones agregadas para la compatibiidad de innobusMX

	DESFire::MIFARE_DESFIRE_CreateApp
	DESFire::MIFARE_DESFIRE_CreateStdDataFile
	DESFire::MIFARE_DESFIRE_WriteData
	DESFire::MIFARE_DESFIRE_ReadDataInno
	DESFire::MIFARE_DESFIRE_InicializarDesfireAppInobusmx
	DESFire::MIFARE_DESFIRE_WriteDataInnoParametros
	DESFire::MIFARE_DESFIRE_ReadDataInnoParametros

*/

DESFire::StatusCode DESFire::MIFARE_DESFIRE_FormatPICC(mifare_desfire_tag *tag)
{

	byte ahuten[1] = {0X0F};
	byte ahutenSize = 1;
	StatusCode formaResult;
	formaResult = MIFARE_BlockExchange(tag, 0x0A, ahuten, &ahutenSize);
	if (IsStatusCodeOK(formaResult)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la formateada");
	}
	Serial.println(GetStatusCodeName(formaResult));
	//return formaResult;
	...
	result = MIFARE_BlockExchange(tag, 0xFC, buffer, &bufferSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la formateada");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_CreateApp(mifare_desfire_tag *tag)
{
	StatusCode result;
	Serial.println("Creando una app");
	byte buffer[5] = {0X05,0X00,0X00,0X0F,0X01};
	byte bufferSize = 5;

	result = MIFARE_BlockExchangeWithData(tag, 0xCA, buffer, &bufferSize, buffer, &bufferSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la formateada");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_CreateStdDataFile(mifare_desfire_tag *tag)
{
	mifare_desfire_aid_t aid;
	...
	aid.data[2] = 0x00;
	StatusCode result;
	result = MIFARE_DESFIRE_SelectApplication(tag, &aid);
	if (!IsStatusCodeOK(result)) {
		Serial.println(F("Error: Failed to select application."));
		Serial.println(GetStatusCodeName(result));
		Serial.println(F("-------------------------------------------------------------"));
	...
	Serial.println("Creando un archivo en la aplicacion seleccionada");
	byte buffer[7] = {0X04,0X00,0XEE,0XEE,0X20,0X00,0X00};
	byte bufferSize = 7;

	result = MIFARE_BlockExchangeWithData(tag, 0xCD, buffer, &bufferSize, buffer, &bufferSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	}
	Serial.println(GetStatusCodeName(result));
	return result;
}

DESFire::StatusCode DESFire::MIFARE_DESFIRE_WriteData(mifare_desfire_tag *tag)
{
	mifare_desfire_aid_t aid;
	...
	//byte buffer[10] = {0X04,0X00,0X00,0X00,0X03,0X00,0X00,0X01,0X02,0X03};
	byte buffer[10] = {0X04,0X00,0X00,0X00,0X03,0X00,0X00,0X48,0X5A,0X52};
	byte bufferSize = 10;

	result = MIFARE_BlockExchangeWithData(tag, 0x3D, buffer, &bufferSize, buffer, &bufferSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_ReadDataInno(mifare_desfire_tag *tag)
{
	mifare_desfire_aid_t aid;
	StatusCode result;
	...
	//Serial.println(nTS);
	Serial.println("Leyendo archivo 2 de la aplicacion 5");
	byte buffer[7] = {0X04,0X00,0X00,0X00,0X20,0X00,0X00};
	byte bufferSize = 7;
	byte bufferRes[3];
	byte bufferResSize = 3;

	result = MIFARE_BlockExchangeWithData(tag, 0xBD, buffer, &bufferSize, bufferRes, &bufferResSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Que paso");
		}
	...
	for (byte i = 0; i <=2; i++) {
      //nTS += byte(bufferRes[i]);
		Serial.print(i);
		Serial.print("   ");
		Serial.println(bufferRes[i]);
    }

	//Serial.println(bufferRes);


	return result;
}

DESFire::StatusCode DESFire::MIFARE_DESFIRE_InicializarDesfireAppInobusmx(mifare_desfire_tag *tag, byte readString[])
{
	/*
		########################################################################
									Aplicacion numero 1
	...
	*/
	...
	StatusCode result;
	Serial.println("Creando una app");
	byte buffer[5] = {0X01,0X00,0X00,0X0F,0X01};
	byte bufferSize = 5;
	...
	Serial.println("Creando un archivo en la aplicacion seleccionada");
	//file with 60bytes
	byte buffer1[7] = {0X01,0X00,0XEE,0XEE,0X3C,0X00,0X00};
	byte bufferSize1 = 7;

	result = MIFARE_BlockExchangeWithData(tag, 0xCD, buffer1, &bufferSize1, buffer1, &bufferSize1);
	if (IsStatusCodeOK(result)) {
		Serial.println("Respuesta de la creacion del archivo");
	}
	Serial.println(GetStatusCodeName(result));
	...
	readString[0] = file1;
	readString[1] = offset0;
	readString[2] = offset1;
	readString[3] = offset2;
	readString[4] = longi0;
	readString[5] = longi1;
	readString[6] = longi2;
	...
	result = MIFARE_BlockExchangeWithData(tag, 0x3D, readString, &bufferSize2, readString, &bufferSize2);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	...
	/*
		...
		for (int n = 0; n < 28; n++) {
			Serial.println(readString[n],HEX);
			}
		delay(300);
	*/

	Serial.println("Seleccionando root");
	mifare_desfire_aid_t aid3;

	aid3.data[0] = 0x00;
	aid3.data[1] = 0x00;
	aid3.data[2] = 0x00;

	result = MIFARE_DESFIRE_SelectApplication(tag, &aid3);
	if (!IsStatusCodeOK(result)) {
		Serial.println(F("Error: Failed to select application."));
		Serial.println(GetStatusCodeName(result));
		Serial.println(F("-------------------------------------------------------------"));
	...
	Serial.println("Creando una app 2");
	byte bufferApp2[5] = {0X02,0X00,0X00,0X0F,0X01};
	byte bufferSice2 = 5;
	//StatusCode result;
	mifare_desfire_aid_t aid2;

	aid2.data[0] = 0x02;
	aid2.data[1] = 0x00;
	aid2.data[2] = 0x00;

	result = MIFARE_BlockExchangeWithData(tag, 0xCA, bufferApp2, &bufferSice2, bufferApp2, &bufferSice2);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la formateada");
	...
	Serial.println("Valor de la creacion");
	Serial.println(GetStatusCodeName(result));

	result = MIFARE_DESFIRE_SelectApplication(tag, &aid2);
	if (!IsStatusCodeOK(result)) {
		Serial.println(F("Error: Failed to select application."));
		Serial.println(GetStatusCodeName(result));
		Serial.println(F("-------------------------------------------------------------"));
	...
	Serial.println("Creando un archivo en la aplicacion seleccionada");
	//file with 60bytes
	byte bufferFile1[7] = {0X01,0X00,0XEE,0XEE,0X28,0X00,0X00};
	byte bufferSiceFile = 7;

	result = MIFARE_BlockExchangeWithData(tag, 0xCD, bufferFile1, &bufferSiceFile, bufferFile1, &bufferSiceFile);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	...
	Serial.println("Escribiendo en archivo");
	byte lala[13] = {0X01,0X00,0X00,0X00,0X06,0X00,0X00,0X00,0X00,0X00,0X00,0X00,0X00};
	byte bufferApp2SaldoSice = 13;

	result = MIFARE_BlockExchangeWithData(tag, 0x3D, lala, &bufferApp2SaldoSice, lala, &bufferApp2SaldoSice);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_ReadDataInnoParametros(mifare_desfire_tag *tag, byte app[], byte fileData[], byte *saldo)
{
	mifare_desfire_aid_t aid;
	StatusCode result;
	...
	//Serial.println(nTS);
	//Serial.println("Leyendo archivo 1 de la aplicacion 2");
	//byte buffer[7] = {0X04,0X00,0X00,0X00,0X20,0X00,0X00};
	byte bufferSize = 7;
	byte bufferRes[5];
	byte bufferResSize = 5;

	result = MIFARE_BlockExchangeWithData(tag, 0xBD, fileData, &bufferSize, bufferRes, &bufferResSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		//Serial.println("Que paso");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_WriteDataInnoParametros(mifare_desfire_tag *tag, byte app[], byte dataW[], byte longi)
{
	mifare_desfire_aid_t aid;
	...
	//Serial.println(GetStatusCodeName(result));
	//StatusCode result;
	byte bufferSize = longi;

	result = MIFARE_BlockExchangeWithData(tag, 0x3D, dataW, &bufferSize, dataW, &bufferSize);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		//Serial.println("Respuesta de la creacion del archivo");
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_ReadDataInnoParametrosPago(mifare_desfire_tag *tag, byte app[], byte fileData[], byte *retorno, byte longi)
{
	mifare_desfire_aid_t aid;
	StatusCode result;
	...
	//byte buffer[7] = {0X04,0X00,0X00,0X00,0X20,0X00,0X00};
	byte bufferSize = 7;
	byte bufferRes[longi];
	byte bufferResSize = longi;
	...
	//Serial.println(GetStatusCodeName(result));
	//Serial.println("Datos");
	//Serial.println("desfire.ccp");
	for (byte i = 0; i<longi; i++) {
        //nTS += byte(bufferRes[i]);
		//Serial.print(i);
		//Serial.print("   ");
		//Serial.println(bufferRes[i],HEX);
		//bufferRes[i] = saldo[i];
		retorno[i] = bufferRes[i];
    }
		//Serial.println("");
    delay(100);
	//Serial.println(bufferRes);
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_InicializarV2DesfireAppInobusmx(mifare_desfire_tag *tag, byte readString[])
{
	StatusCode result;
	Serial.println("Creando una app");
	byte buffer[5] = {0X01,0X00,0X00,0X0F,0X01};
	...
	Serial.println("Creando un archivo en la aplicacion seleccionada");
	//file with 60bytes
	byte buffer1[7] = {0X01,0X00,0XEE,0XEE,0X78,0X00,0X00};
	byte bufferSize1 = 7;
...
DESFire::StatusCode DESFire::MIFARE_DESFIRE_InicializarV3DesfireAppInobusmx(mifare_desfire_tag *tag, byte llave[], byte tipoTisc[], byte EstadoTarjeta[], byte idTipoTisc[])
{
	StatusCode result;
	Serial.println("Creando una app");
	byte buffer[5] = {0X01,0X00,0X00,0X0F,0X01};
	...
	byte tipoTarjetaWD[9];
	tipoTarjetaWD[0] = file1tt;
	tipoTarjetaWD[1] = offset0tt;
	tipoTarjetaWD[2] = offset1tt;
	tipoTarjetaWD[3] = offset2tt;
	tipoTarjetaWD[4] = longi0tt;
	tipoTarjetaWD[5] = longi1tt;
	tipoTarjetaWD[6] = longi2tt;

	for (int n = 0; n < 2; n++) {
			tipoTarjetaWD[n+7] = tipoTisc[n];
		}
	delay(300);
	...
	result = MIFARE_BlockExchangeWithData(tag, 0x3D, tipoTarjetaWD, &bufferSize3, tipoTarjetaWD, &bufferSize3);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	...
	byte file1ttt   = 0X01;
	byte offset0ttt = 0X18;
	byte offset1ttt = 0X00;
	byte offset2ttt = 0X00;
	byte longi0ttt  = 0X01;
	byte longi1ttt  = 0X00;
	byte longi2ttt  = 0X00;

	byte estadoTarjetaWD[8];
	estadoTarjetaWD[0] = file1ttt;
	estadoTarjetaWD[1] = offset0ttt;
	estadoTarjetaWD[2] = offset1ttt;
	estadoTarjetaWD[3] = offset2ttt;
	estadoTarjetaWD[4] = longi0ttt;
	estadoTarjetaWD[5] = longi1ttt;
	estadoTarjetaWD[6] = longi2ttt;

	for (int n = 0; n < 1; n++) {
			estadoTarjetaWD[n+7] = EstadoTarjeta[n];
		}
	delay(300);
	...
	result = MIFARE_BlockExchangeWithData(tag, 0x3D, estadoTarjetaWD, &bufferSize4, estadoTarjetaWD, &bufferSize4);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	...
	byte file1tttt   = 0X01;
	byte offset0tttt = 0X20;
	byte offset1tttt = 0X00;
	byte offset2tttt = 0X00;
	byte longi0tttt  = 0X02;
	byte longi1tttt  = 0X00;
	byte longi2tttt  = 0X00;

	byte idTipoTiscWD[9];
	idTipoTiscWD[0] = file1tttt;
	idTipoTiscWD[1] = offset0tttt;
	idTipoTiscWD[2] = offset1tttt;
	idTipoTiscWD[3] = offset2tttt;
	idTipoTiscWD[4] = longi0tttt;
	idTipoTiscWD[5] = longi1tttt;
	idTipoTiscWD[6] = longi2tttt;

	for (int n = 0; n < 2; n++) {
			idTipoTiscWD[n+7] = idTipoTisc[n];
		}
	delay(300);
	...
	result = MIFARE_BlockExchangeWithData(tag, 0x3D, idTipoTiscWD, &bufferSize5, idTipoTiscWD, &bufferSize5);
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la creacion del archivo");
	...
	*/
	...
	if (IsStatusCodeOK(result)) {
		// keep track of the application
		//memcpy(tag->selected_application, aid->data, MIFARE_AID_SIZE);
		Serial.println("Respuesta de la f	ormateada");
	}
